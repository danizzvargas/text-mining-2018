...
embedding_matrix = np.random.random((Config.TOP_WORDS, Config.EMBEDDING_VECTOR_LEN))
...
model.add(Conv1D(128, 5, activation='relu',kernel_regularizer=regularizers.l2(0.02)))
...
print(model.summary())
history = model.fit(X_train, y_train, epochs=5,shuffle=True, batch_size=64,validation_data=(X_validate, y_validate))

predictions = model.predict(X_validate).round().astype('int')
print('\nResults in validation Data')
print('Accuracy {}'.format(accuracy_score(y_validate,predictions.round())))
print('Precision {}'.format(precision_score(y_validate,predictions.round())))
print('Recall {}'.format(recall_score(y_validate,predictions.round())))
print('F1 {}'.format(f1_score(y_validate,predictions.round())))
